...
impl<S: ?Sized> CachedService<S> {
    ...
    where
        F: FnOnce() -> Fut,
        Fut: Future<Output = IngressResult<Arc<S>>>,
    {
        {
...
impl<S: ?Sized, C: PartialEq + Clone> ConfiguredCache<S, C> {
    ...
    {
        {
            let sg = self.service.read();
            let cg = self.config.read();
            if let (Some(ref s), Some(ref c)) = (&*sg, &*cg) {
                if c == config {
                    return Ok(Arc::clone(s));
                }
            }
        ...
        {
            let mut sg = self.service.write();
            let mut cg = self.config.write();
            *sg = Some(Arc::clone(&new_service));
            *cg = Some(config.clone());
        }
        Ok(new_service)
    }
...
mod tests {
    ...
    async fn test_get_or_init_initializes_and_caches_service() {
        ...
        let r = cache
            .get_or_init(|| async { Ok(Arc::new(Svc { val: 42 })) })
            .await
            .unwrap();
        assert_eq!(r.val, 42);
        assert!(cache.is_initialized());
    ...
    async fn test_reset_clears_cached_service() {
        let cache: CachedService<Svc> = CachedService::new();
        cache
            .get_or_init(|| async { Ok(Arc::new(Svc { val: 1 })) })
            .await
            .unwrap();
        cache.reset();
        assert!(!cache.is_initialized());
    ...
    async fn test_get_or_init_with_config_reinitializes_on_config_change() {
        ...
        let first = cache
            .get_or_init_with_config(&"cfg1".into(), |_| async { Ok(Arc::new(Svc { val: 1 })) })
            .await
            .unwrap();
        assert_eq!(first.val, 1);
        let third = cache
            .get_or_init_with_config(&"cfg2".into(), |_| async { Ok(Arc::new(Svc { val: 3 })) })
            .await
            .unwrap();
    ...
    async fn test_current_config_returns_none_before_init() {
        ...
        cache
            .get_or_init_with_config(&"my_cfg".into(), |_| async { Ok(Arc::new(Svc { val: 1 })) })
            .await
            .unwrap();
