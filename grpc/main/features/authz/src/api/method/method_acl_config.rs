...
mod tests {
    ...
    /// @covers: allow_for_any_authenticated
    #[test]
    fn test_allow_for_any_authenticated_allows_any_subject() {
        let cfg = MethodAclConfig::deny_all().allow_for_any_authenticated("/svc/Health");
        assert!(cfg.allows(Some("charlie"), "/svc/Health"));
        assert!(cfg.allows(Some("dave"), "/svc/Health"));
        assert!(!cfg.allows(None, "/svc/Health"));
    }
