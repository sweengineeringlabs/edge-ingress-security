...
pub fn decode_request(body: &[u8]) -> Result<ReflectionRequest, ReflectionError> {
    ReflectionCodec::decode_request(body)
}

pub fn encode_response(response: &ReflectionResponse, original_request: &[u8]) -> Vec<u8> {
    ReflectionCodec::encode_response(response, original_request)
}

pub(crate) fn decode_varint(bytes: &[u8]) -> Option<(u64, usize)> {
    ReflectionCodec::decode_varint(bytes)
}

pub(crate) fn encode_varint(value: u64, out: &mut Vec<u8>) {
    ReflectionCodec::encode_varint(value, out)
}
...
mod tests {
    ...
    /// @covers: ReflectionCodec::decode_varint, ReflectionCodec::encode_varint — round-trip.
    #[test]
    ...
    /// @covers: ReflectionCodec::decode_request — empty body returns Unknown.
    #[test]
    ...
    /// @covers: ReflectionCodec::decode_request — list_services field=7.
    #[test]
    ...
    /// @covers: ReflectionCodec::decode_request — file_by_filename field=3.
    #[test]
