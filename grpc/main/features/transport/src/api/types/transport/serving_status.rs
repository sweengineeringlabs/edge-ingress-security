//! Wire-equivalent serving status enum.

/// Wire-equivalent of `grpc.health.v1.HealthCheckResponse.ServingStatus`.
///
/// Returned by health check handlers and used by [`HealthAggregate`] to
/// compute the combined status of all registered services.
///
/// [`HealthAggregate`]: crate::HealthAggregate
///
/// # Examples
///
/// ```rust
/// use swe_edge_ingress_grpc_transport::ServingStatus;
///
/// assert_eq!(ServingStatus::Serving as i32, 1);
/// assert_eq!(ServingStatus::NotServing as i32, 2);
/// assert_ne!(ServingStatus::Serving, ServingStatus::Unknown);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(i32)]
pub enum ServingStatus {
    /// Service status is unknown.
    Unknown = 0,
    /// Service is serving requests.
    Serving = 1,
    /// Service is not serving requests.
    NotServing = 2,
    /// Named service is unknown to the health reporter.
    ServiceUnknown = 3,
}
