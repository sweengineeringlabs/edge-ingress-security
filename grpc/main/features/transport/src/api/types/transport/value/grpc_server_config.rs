...
mod tests {
    ...
    fn test_allow_unauthenticated_sets_the_flag() {
        let cfg = GrpcServerConfig::new(addr()).allow_unauthenticated();
        assert!(cfg.allow_unauthenticated);
    }
    ...
    /// @covers: with_tls
    #[test]
    fn test_with_tls_attaches_tls_config() {
        use swe_edge_ingress_tls::IngressTlsConfig;
        let cfg = GrpcServerConfig::new(addr()).with_tls(IngressTlsConfig::tls("c.pem", "k.pem"));
    ...
    fn test_with_max_message_bytes_overrides_default() {
        let cfg = GrpcServerConfig::new(addr()).with_max_message_bytes(1024);
    ...
    fn test_with_max_concurrent_streams_overrides_default() {
        let cfg = GrpcServerConfig::new(addr()).with_max_concurrent_streams(50);
