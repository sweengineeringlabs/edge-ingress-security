...
impl AxumHttpServerHelper {
    ...
    pub async fn dispatch_sse(
        req: axum::extract::Request,
        body_limit: usize,
        handler: Arc<dyn crate::api::port::http::http_stream::HttpStream>,
    ) -> axum::response::Response {
...
mod tests {
    ...
    fn test_map_method_maps_all_standard_http_verbs() {
        assert_eq!(
            AxumHttpServerHelper::map_method(&axum::http::Method::GET),
            HttpMethod::Get
        );
        assert_eq!(
            AxumHttpServerHelper::map_method(&axum::http::Method::POST),
            HttpMethod::Post
        );
        assert_eq!(
            AxumHttpServerHelper::map_method(&axum::http::Method::PUT),
            HttpMethod::Put
        );
        assert_eq!(
            AxumHttpServerHelper::map_method(&axum::http::Method::PATCH),
            HttpMethod::Patch
        );
        assert_eq!(
            AxumHttpServerHelper::map_method(&axum::http::Method::DELETE),
            HttpMethod::Delete
        );
        assert_eq!(
            AxumHttpServerHelper::map_method(&axum::http::Method::HEAD),
            HttpMethod::Head
        );
        assert_eq!(
            AxumHttpServerHelper::map_method(&axum::http::Method::OPTIONS),
            HttpMethod::Options
        );
    }
    ...
    fn test_parse_query_extracts_key_value_pairs() {
        let q = AxumHttpServerHelper::parse_query(Some("foo=bar&baz=qux"));
    ...
    fn test_parse_query_returns_empty_map_for_none() {
        assert!(AxumHttpServerHelper::parse_query(None).is_empty());
    }

    #[test]
    fn test_parse_query_handles_value_with_equals_sign() {
        let q = AxumHttpServerHelper::parse_query(Some("token=a=b"));
        assert_eq!(q.get("token"), Some(&"a=b".to_string()));
    }

    #[test]
    fn test_parse_query_decodes_percent_encoded_keys_and_values() {
        let q = AxumHttpServerHelper::parse_query(Some("na%20me=val%3D1"));
    ...
    fn test_build_body_returns_none_for_empty_bytes() {
        assert!(
            AxumHttpServerHelper::build_body(&bytes::Bytes::new(), "application/json").is_none()
        );
    }
    ...
    fn test_build_body_parses_valid_json_content_type_as_json_variant() {
        ...
        assert!(matches!(
            AxumHttpServerHelper::build_body(&bytes, "application/json"),
    ...
    fn test_build_body_parses_form_encoded_content_type_as_form_variant() {
        ...
        assert!(matches!(
            AxumHttpServerHelper::build_body(&bytes, "application/x-www-form-urlencoded"),
    ...
    fn test_build_body_uses_raw_for_octet_stream_content_type() {
        ...
        assert!(matches!(
            AxumHttpServerHelper::build_body(&bytes, "application/octet-stream"),
            Some(HttpBody::Raw(_))
        ));
    }
    ...
    fn test_parse_form_decodes_plus_as_space() {
        ...
        let m = AxumHttpServerHelper::parse_form(&b);
    ...
    fn test_build_response_maps_200_status_code() {
        let resp = AxumHttpServerHelper::build_response(HttpResponse::new(200, b"ok".to_vec()));
        assert_eq!(resp.status(), axum::http::StatusCode::OK);
    }

    #[test]
    fn test_build_response_maps_404_status_code() {
        let resp = AxumHttpServerHelper::build_response(HttpResponse::new(404, vec![]));
        assert_eq!(resp.status(), axum::http::StatusCode::NOT_FOUND);
    }

    #[test]
    fn test_build_response_falls_back_to_500_for_out_of_range_status() {
        let resp = AxumHttpServerHelper::build_response(HttpResponse::new(1000, vec![]));
        assert_eq!(resp.status(), axum::http::StatusCode::INTERNAL_SERVER_ERROR);
    }

    #[test]
    fn test_error_response_maps_not_found_to_404() {
        let r = AxumHttpServerHelper::error_response(HttpIngressError::NotFound("x".into()));
        assert_eq!(r.status(), axum::http::StatusCode::NOT_FOUND);
    }

    #[test]
    fn test_error_response_maps_invalid_input_to_400() {
        let r = AxumHttpServerHelper::error_response(HttpIngressError::InvalidInput("bad".into()));
    ...
    fn test_percent_decode_decodes_hex_encoded_slash() {
        assert_eq!(AxumHttpServerHelper::percent_decode("%2F"), "/");
    }

    #[test]
    fn test_percent_decode_passes_through_invalid_percent_sequence() {
        assert_eq!(AxumHttpServerHelper::percent_decode("%ZZ"), "%ZZ");
    }
}
