...
fn load_key(path: &str) -> Result<PrivateKeyDer<'static>, IngressTlsError> {
    let file = fs::File::open(path).map_err(|e| IngressTlsError::KeyLoad(path.to_string(), e))?;
...
mod tests {
    ...
    /// @covers: build_acceptor — succeeds with valid TLS config.
    #[test]
    ...
    /// @covers: build_acceptor — returns CertLoad error for missing cert file.
    #[test]
