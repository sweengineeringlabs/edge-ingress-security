...
fn make_token(secret: &[u8], sub: &str, exp_offset_secs: i64) -> String {
    ...
    let now = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_secs();
    let raw = Raw { sub: sub.into(), exp: (now as i64 + exp_offset_secs) as u64 };
...
fn test_jwt_verifier_iss_mismatch_returns_claim_mismatch() {
    use jsonwebtoken::{encode, EncodingKey, Header};
    #[derive(serde::Serialize)]
    struct Raw { sub: String, iss: String, exp: u64 }
    ...
    let token = encode(
        &Header::default(),
        &Raw { sub: "u".into(), iss: "wrong.issuer".into(), exp: now + 3600 },
